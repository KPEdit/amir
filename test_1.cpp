...
TEST(EvolventTest, PolarEquationTest)
{
    ...
    const char *tmp = "r^2 = 0, dot with coordinates(0, 0)";
    line.change_params(0);
    t = line.get_polar_equation();
    EXPECT_STRCASEEQ(t, tmp);
    delete t;

    tmp = "r^2 = 4*(1 + phi^2), with center in (0, 0)";
    line.change_params(2);
    t = line.get_polar_equation();
    EXPECT_STRCASEEQ(t, tmp);
    delete t;
    ...
    tmp = "r^2 = 0, dot with coordinates(2, 3)";
    line.change_params(0);
    t = line.get_polar_equation();
    EXPECT_STRCASEEQ(t, tmp);
    delete t;

    tmp = "r^2 = 1*(1 + phi^2), with center in (2, 3)";
